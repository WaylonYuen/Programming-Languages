...
namespace Server_v1.Network {
    ...
    public class Send {
        ...
        /// <param name="player">玩家類</param>
        /// <param name="databaseType">資料型態</param>
        /// <param name="data_Byte">資料內容</param>
        public void GameDataPacket(Player player, DataBaseType databaseType, byte[] data_Byte) {
            byte[] Packet = Build_Package(player.Crccode, PackageType.GameData, databaseType, DataType.ByteData, player.EncryptionMethods, player.SessionID, data_Byte);
            Send_Packet(player.Socket, Packet);
        }
        #endregion
