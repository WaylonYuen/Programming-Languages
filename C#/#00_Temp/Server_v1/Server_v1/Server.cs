...
using Server_v1.Database;
using Server_v1.Network;
using Server_v1.Network.CallBack;
...
namespace Server_v1 {
    ...
    public class Server {
        ...
        private Database_CallBackMethods DB_CallBackMethods = new Database_CallBackMethods();   //創建數據庫回調類
        ...
        public Server() {
            ...
            Packet_CallBackMethods.RegisterCallBackMethods();                       //註冊封包回調方法
        ...
        private void Alive_Thread(object ClientSocket) {
            ...
            while (!isQuit) {
                Thread.Sleep(30000);    //每隔30秒,發送一次心跳包
                if (player.IsConnected && player.Responese) {
                    ...
                    Console.WriteLine("#  偵測不到玩家的回覆, 執行斷線程序.");
        ...
        private void DB_CallBack_Thread() {
            ...
            //開啟數據庫連線
            Database_Start dataBase = new Database_Start(); //創建數據庫連接
        ...
        private void PackageSetup(Player player) {
            ...
            Send.BoolPacket(player, PackageType.None, false);
            #endregion
    ...
    public class ReceivePacket {
        ...
        public void CheckPacket(Player player, byte[] Head_Bytes, byte[] Body_Bytes) {
            ...
            bool canEntry = (isIDKey && isCrccode) || PackageType.Login == packageType || DataBaseType.Longin == dataBaseType; //登陸封包可以直接通過
